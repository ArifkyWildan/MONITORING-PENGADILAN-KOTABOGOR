...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <h1 class="text-2xl font-bold text-gray-900">
            <i class="fas fa-edit mr-2 text-blue-600"></i>Edit Isian
        </h1>
        <p class="text-gray-600 mt-1">Perbarui data isian</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('isian.update', $isian) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <!-- Nama Bagian -->
                ...
                <div>
                    ...
                    <textarea name="daftar_isi" rows="8" required placeholder="Masukkan daftar isi..." class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 @error('daftar_isi') border-red-500 @enderror">{{ old('daftar_isi', $isian->daftar_isi) }}</textarea>
                    @error('daftar_isi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div>
                            <p class="text-sm text-yellow-700">
                                <strong>Perhatian:</strong> Perubahan data tidak akan mempengaruhi link dan verifikasi yang sudah ada.
                            </p>
                        </div>
                    </div>
                </div>
