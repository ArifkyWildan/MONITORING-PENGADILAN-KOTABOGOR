        ...
        <div class="bg-gradient-to-r from-red-800 to-red-900 rounded-xl shadow-2xl p-8 mb-6 relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-red-100 text-lg ml-1">Perbarui data isian yang sudah ada</p>
                </div>
                <div class="hidden md:block">
                    <div class="bg-white bg-opacity-10 rounded-full p-4">
                        <i class="fas fa-file-edit text-5xl text-white opacity-80"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-red-700 to-red-800 px-8 py-4">
                <h2 class="text-xl font-semibold text-white flex items-center">
                    ...
                    Form Edit Data Isian
                </h2>
            </div>

            <!-- Form Body -->
            <form action="{{ route('isian.update', $isian) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                <div class="space-y-8">
                    <!-- Nama Bagian -->
                    <div class="group">
                        ...
                        <select name="bagian_id" required class="w-full px-4 py-4 border-2 border-gray-300 rounded-xl shadow-sm focus:border-red-600 focus:ring-4 focus:ring-red-100 transition-all duration-200 @error('bagian_id') border-red-500 @enderror">
                            ...
                            <option value="{{ $bagian->id }}" {{ (old('bagian_id', $isian->bagian_id) == $bagian->id) ? 'selected' : '' }}>
                                {{ $bagian->nama_bagian }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-500 rounded-lg p-5">
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                <h3 class="text-sm font-bold text-yellow-900 mb-2">Perhatian!</h3>
                                <p class="text-sm text-yellow-800">
                                    Perubahan data tidak akan mempengaruhi link dan verifikasi yang sudah ada. Pastikan Anda memeriksa kembali data sebelum menyimpan perubahan.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-blue-500 rounded-lg p-5">
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                <h3 class="text-sm font-bold text-blue-900 mb-1">Tips Pengeditan</h3>
                                <ul class="text-sm text-blue-800 space-y-1">
                                    <li class="flex items-start">
                                        <i class="fas fa-check-circle text-blue-600 mr-2 mt-0.5"></i>
                                        <span>Periksa kembali bagian yang dipilih</span>
                                    </li>
                                    <li class="flex items-start">
                                        <i class="fas fa-check-circle text-blue-600 mr-2 mt-0.5"></i>
                                        <span>Update daftar isi sesuai perubahan terbaru</span>
                                    </li>
                                    <li class="flex items-start">
                                        <i class="fas fa-check-circle text-blue-600 mr-2 mt-0.5"></i>
                                        <span>Simpan perubahan setelah selesai mengedit</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
